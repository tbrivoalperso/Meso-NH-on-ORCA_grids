...
DO JI=1,IIU
DO JJ=1,IJU
TEMP2D_PDXHAT(JI,JJ) = PDXHAT2D(JI,JJ)
TEMP2D_PDYHAT(JI,JJ) = PDYHAT2D(JI,JJ)
...
IF (.NOT.LCARTESIAN) THEN
  ...
  DO JK=1,IKU ; DO JJ=1,IJU ; DO JI=1,IIU
    ...
    PDXX(JI,JJ,JK) = ZDZZ(JI,JJ,JK) * PDXHAT2D(JI,JJ) /PMAP(JI,JJ)
    PDYY(JI,JJ,JK) = ZDZZ(JI,JJ,JK) * PDYHAT2D(JI,JJ) /PMAP(JI,JJ)
!     PDXX(JI,JJ,JK) = PDXHAT2D(JI,JJ) /PMAP(JI,JJ)
 
!     PDYY(JI,JJ,JK) = PDYHAT2D(JI,JJ) /PMAP(JI,JJ)
...
ELSE
  DO JK=1,IKU ; DO JJ=1,IJU ; DO JI=1,IIU
    PDXX(JI,JJ,JK) = PDXHAT2D(JI,JJ)
    PDYY(JI,JJ,JK) = PDYHAT2D(JI,JJ)
  ENDDO ; ENDDO ; ENDDO
  PDXX(:,:,:)=MXM(PDXX(:,:,:))
  PDYY(:,:,:)=MYM(PDYY(:,:,:))
  PRINT*,'YOUR COMPUTATION IS FUCKED?'

...
!-------------------------------------------------------------------------------
!      ####################################################################
       SUBROUTINE SM_GRIDPROJ(HLUOUT,PXHAT,PYHAT,PZHAT,PXHAT2D,PYHAT2D,PZS,           &
                              OSLEVE,PLEN1,PLEN2,PZSMT,PLATOR,PLONOR, &
                              PLAT,PLON,plonu,platv,platt,PMAP, &
			      PDXHAT,PDYHAT,PDXHAT2D,PDYHAT2D,PZZ,PJ     )
!      ####################################################################
!
...
REAL, DIMENSION(:,:),      INTENT(IN) :: PXHAT2D,PYHAT2D ! Positions x,y,z in 
                                                         ! the cartesian plane
...
REAL,                    INTENT(IN) :: PLONOR            ! Longitude of the 
                                                         ! origine point 
REAL, DIMENSION(:,:),   INTENT(INOUT) :: PLAT,PLON         ! Latitude and longitude
                                                         ! of mass gridpoints
REAL, DIMENSION(:,:),   INTENT(INOUT) :: plonu
REAL, DIMENSION(:,:),   INTENT(INOUT) :: platv
REAL, DIMENSION(:,:),   INTENT(INOUT) :: platt  !lat / lon u,t,v grid 
REAL, DIMENSION(:,:),   INTENT(INOUT)  :: PDXHAT2D,PDYHAT2D         ! Latitude and longitude
                                                         ! of mass gridpoints
...
REAL, DIMENSION(SIZE(PLAT,1),SIZE(PLAT,2)) :: DLATV2D,DLONV2D,DLATU2D,DLONU2D         ! Latitude and longitude
                                                         ! of mass gridpoints
...
DO JJ=1,IJU ;
  DO JI=1,IIU ;
    ZXHATM(1:IIU-1,JJ) = .5*(PXHAT2D(1:IIU-1,JJ)+PXHAT2D(2:IIU,JJ))   
    ZXHATM(IIU,JJ)     = 2.*PXHAT2D(IIU,JJ)-ZXHATM(IIU-1,JJ)
  ENDDO;
ENDDO;
...
DO JJ=1,IJU ;
  DO JI=1,IIU ;
     ...
     ZYHATM(JI,1:IJU-1) = .5*(PYHAT2D(JI,1:IJU-1)+PYHAT2D(JI,2:IJU))
     ZYHATM(JI,IJU)     = 2.*PYHAT2D(JI,IJU)-ZYHATM(JI,IJU-1)
...
!		COMPUTE 2D DXHAT   - EDIT THEO BRIVOAL
DO JJ=1,IJU ;
DLONU2D(1:IIU-1,JJ)=plonu(2:IIU,JJ)-plonu(1:IIU-1,JJ)
DLONU2D(IIU,JJ)      = DLONU2D(IIU-1,JJ)
PDXHAT2D(1:IIU-1,JJ)  = PXHAT2D(2:IIU,JJ) - PXHAT2D(1:IIU-1,JJ)
PDXHAT2D(IIU,JJ)      = PDXHAT2D(IIU-1,JJ)
ENDDO;
!

DO JI=1,IIU ;
DLATV2D(JI,1:IJU-1)  = platv(JI,2:IJU) - platv(JI,1:IJU-1)
DLATV2D(JI,IJU)      = DLATV2D(JI,IJU-1)

PDYHAT2D(JI,1:IJU-1)  = PYHAT2D(JI,2:IJU) - PYHAT2D(JI,1:IJU-1)
PDYHAT2D(JI,IJU)      = PDYHAT2D(JI,IJU-1)
ENDDO;

!PDXHAT2D(:,:)=ABS(DLONU2D(:,:)*(2.*XPI*XRADIUS/360.)*COS(platt(:,:)*XPI/180.))

!PDYHAT2D(:,:)=ABS(DLATV2D(:,:)*(2.*XPI*XRADIUS/360.))
!
...
CALL ADD2DFIELD_ll(TZHALO_ll,PDXHAT2D)
CALL ADD2DFIELD_ll(TZHALO_ll,PDYHAT2D)
CALL UPDATE_HALO_ll(TZHALO_ll,IINFO_ll)
...
!DO JJ=1,IJU ; DO JI=1,IIU

!
!PMAP(JI,JJ)=(PDXHAT2D(IIU/2,JJ) * PDYHAT2D(JI,IJU/2))/(PDXHAT2D(JI,JJ) * PDYHAT2D(JI,JJ))
!PMAP(JI,JJ)=(DLAT2D(IIU/2,IJU/2)*DLON2D(IIU/2,IJU/2)*COS(PLAT(IIU/2,IJU/2)*XPI/180.))/(DLAT2D(JI,JJ)*DLON2D(JI,JJ)*COS(PLAT(JI,JJ)*XPI/180.))
!PMAP(JI,JJ)=(DLAT2D(IIU/2,IJU/2)*DLON2D(IIU/2,IJU/2)*COS(PLAT(IIU/2,IJU/2)*XPI/180.)*((2.*XPI*XRADIUS/360.)**2))&
 !     /(PDXHAT2D(JI,JJ) * PDYHAT2D(JI,JJ))
!PMAP(JI,JJ)=(24416*20535)/(DLAT2D(JI,JJ)*DLON2D(JI,JJ)*COS(PLAT(JI,JJ)*XPI/180.)*((2.*XPI*XRADIUS/360.)**2))

!PMAP(JI,JJ)=(DLAT2D(JI,JJ)*DLON2D(JI,JJ)*COS(PLAT(JI,JJ)*XPI/180.)*((2.*XPI*XRADIUS/360.)**2))/(PDXHAT2D(JI,JJ) * PDYHAT2D(JI,JJ))
!PMAP(JI,JJ)=(24416*20535)/(PDXHAT2D(JI,JJ) * PDYHAT2D(JI,JJ))
...
DO JK=1,IKU ; DO JJ=1,IJU ; DO JI=1,IIU
 PJ(JI,JJ,JK)  = ZAPZOA2(JI,JJ,JK) * (1.0/PMAP(JI,JJ)**2)  &
               * PDXHAT2D(JI,JJ) * PDYHAT2D(JI,JJ) * ZDZ(JI,JJ,JK) 
! PJ(JI,JJ,JK)  = ZAPZOA2(JI,JJ,JK) * PDXHAT2D(JI,JJ) &
!                * PDYHAT2D(JI,JJ) * ZDZ(JI,JJ,JK) 
!PJ(JI,JJ,JK)  = ZAPZOA2(JI,JJ,JK) * (1.0/PMAP(JI,JJ)**2)  &
!               *(24416*20535)* ZDZ(JI,JJ,JK)

...
REAL, DIMENSION(SIZE(PDXHAT2D,1),SIZE(PDXHAT2D,2))  :: ZDXHAT2DM   
  ! Stretching in x direction (mass point)
REAL, DIMENSION(SIZE(PDXHAT2D,1),SIZE(PDXHAT2D,2))  :: ZDYHAT2DM   
  ! Stretching in y direction (mass point)
...
REAL, DIMENSION(SIZE(PRHODJ,1),SIZE(PRHODJ,2)) :: ZGAMMA_o


REAL, DIMENSION(SIZE(PRHODJ,1),SIZE(PRHODJ,2)) :: XDXDXHAT,XDYDYHAT 
REAL, DIMENSION(SIZE(PRHODJ,1),SIZE(PRHODJ,2)) :: PCURVX_o,PCURVY_o
...
! 
ZCDR = XPI/180.
IIU = UBOUND(PDXHAT2D,1)
IJU = UBOUND(PDYHAT2D,2)
DO JJ=1,IJU ;
  DO JI=1,IIU ;
   
 ZDXHAT2DM(1:IIU-1,JJ) = .5*(PDXHAT2D(1:IIU-1,JJ)+PDXHAT2D(2:IIU,JJ))
    ZDXHAT2DM(IIU,JJ)     = 2.*PDXHAT2D(IIU,JJ)-ZDXHAT2DM(IIU-1,JJ)
  ENDDO;
ENDDO;

DO JJ=1,IJU ;
  DO JI=1,IIU ;
    ZDYHAT2DM(JI,1:IJU-1) = .5*(PDYHAT2D(JI,1:IJU-1)+PDYHAT2D(JI,2:IIU))
    ZDYHAT2DM(JI,IJU)     = 2.*PDYHAT2D(JI,IJU)-ZDYHAT2DM(JI,IJU-1)
  ENDDO;
ENDDO;
...
IF (.NOT.LCARTESIAN) THEN
 ! ZGAMMA_o(:,:) = XRPK * ((PLON(:,:) - XLON0)*ZCDR) - (XBETA*ZCDR)
  
  ZGAMMA(:,:)=ATAN(((xlont(:,:)-xlonv(:,:))*COS(xlatt(:,:)*(XPI/180)))/(xlatt(:,:)-xlatv(:,:)))
...
!EDIT-TBRIVOAL
  ...
  !PCURVX(:,:)   = COS(ZGAMMA_o(:,:)) * (SIN(PLAT(:,:)*ZCDR) -XRPK)         &
  !               / COS(PLAT(:,:)*ZCDR)
  !PCURVY(:,:)   = SIN(ZGAMMA_o(:,:)) * (SIN(PLAT(:,:)*ZCDR) -XRPK)         &
  !               / COS(PLAT(:,:)*ZCDR)


  DO JJ=1,IJU ;

    XDXDXHAT(1,JJ)  = PDXHAT2D(2,JJ) - PDXHAT2D(1,JJ)
    XDXDXHAT(2:IIU-1,JJ)  = (PDXHAT2D(3:IIU,JJ) - PDXHAT2D(1:IIU-2,JJ))*0.5
    XDXDXHAT(IIU,JJ)      = PDXHAT2D(IIU,JJ) - PDXHAT2D(IIU-1,JJ)
  ENDDO;

  DO JI=1,IIU ;

    XDYDYHAT(JI,1)  = PDYHAT2D(JI,2) - PDYHAT2D(JI,1)
    XDYDYHAT(JI,2:IJU-1)  = (PDYHAT2D(JI,3:IJU) - PDYHAT2D(JI,1:IJU-2))*0.5
    XDYDYHAT(JI,IJU)      = PDYHAT2D(JI,IJU)-PDYHAT2D(JI,IJU-1)
  ENDDO;


  PCURVX (:,:)   = -(1/(PDYHAT2D*PDYHAT2D))*XDYDYHAT*XRADIUS
  PCURVY (:,:)   = (1/(PDXHAT2D*PDXHAT2D))*XDXDXHAT*XRADIUS
...
!           PRHODJ,PTHVREF,PZZ,PBFY)
  CALL TRIDZ(HLUOUT,HLBCX,HLBCY,                                          &
            PMAP,PDXHAT2D,PDYHAT2D,PDXHATM,PDYHATM,PRHOM,PAF,                 &
            PCF,PTRIGSX,PTRIGSY,KIFAXX,KIFAXY,                            &

...
CONTAINS 
  ...
  SUBROUTINE INITIO_ll()
    ...
    IF (.NOT. LOASIS) THEN
      ...
      END IF
      PRINT*,"Now MPI is initialized for sure"
    END IF
  ...
  USE MODD_IO_ll
    ...
    INTEGER(KIND=8), INTENT(IN),  OPTIONAL :: KMELEV    
    ...
    INTEGER(KIND=8)                 :: IRESOU,IMELEV,INPRAR
    INTEGER(KIND=8)                 :: INUMBR8,ININAR8    
    LOGICAL(KIND=8)                 :: GNAMFI8,GFATER8,GSTATS8 
    !JUANZ
...
!!$    END IF
  ...
  FUNCTION io_file(k,nb_proc_io)
    ...
    !
    IMPLICIT NONE
    INTEGER(kind=4)                   :: k,nb_proc_io
    INTEGER(kind=4)                   :: io_file
  ...
  FUNCTION io_rank(ifile,nb_proc,nb_proc_io,offset_rank)
    ...
    !
    IMPLICIT NONE
    INTEGER(kind=4)                  :: ifile,nb_proc,nb_proc_io
    INTEGER(kind=4),OPTIONAL         :: offset_rank

    INTEGER(kind=4)                  :: io_rank

    INTEGER(kind=4)                  :: ipas,irest

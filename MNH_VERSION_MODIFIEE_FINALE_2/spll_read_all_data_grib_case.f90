...
REAL, DIMENSION(:), ALLOCATABLE    :: ZLONUOUT       ! mapping PGD -> Grib (lon.)
REAL, DIMENSION(:), ALLOCATABLE    :: ZLATUOUT       ! mapping PGD -> Grib (lat.)
REAL, DIMENSION(:), ALLOCATABLE    :: ZLONVOUT       ! mapping PGD -> Grib (lon.)
REAL, DIMENSION(:), ALLOCATABLE    :: ZLATVOUT       ! mapping PGD -> Grib (lat.)
REAL, DIMENSION(:), ALLOCATABLE    :: ZLONTOUT       ! mapping PGD -> Grib (lon.)
REAL, DIMENSION(:), ALLOCATABLE    :: ZLATTOUT       ! mapping PGD -> Grib (lat.)
REAL, DIMENSION(:), ALLOCATABLE    :: ZDXHATOUT       ! mapping PGD -> Grib (lat.)
...
REAL, DIMENSION(:,:), ALLOCATABLE   :: ZDXHAT2D         ! Lat of PGD mass points

REAL, DIMENSION(:,:), INTENT(IN) :: platu         ! Lat of PGD u points
REAL, DIMENSION(:,:), INTENT(IN) :: plonu         ! Lon of PGD u points
REAL, DIMENSION(:,:), INTENT(IN) :: platv         ! Lat of PGD v points
REAL, DIMENSION(:,:), INTENT(IN) :: plonv         ! Lon of PGD v points
...
DO JJ = 1, IJU
  DO JI = 1, IIU
     ZXM(1:IIU-1,JJ) = (PXHAT2D(1:IIU-1,JJ) + PXHAT2D(2:IIU,JJ) ) / 2.
     ZXM(IIU,JJ)     = PXHAT2D(IIU,JJ) - PXHAT2D(IIU-1,JJ) + ZXM(IIU-1,JJ)

     ZYM(JI,1:IJU-1) = (PYHAT2D(JI,1:IJU-1) + PYHAT2D(JI,2:IJU)) / 2.
     ZYM(JI,IJU)     = PYHAT2D(JI,IJU) - PYHAT2D(JI,IJU-1) + ZYM(JI,IJU-1)
...
ALLOCATE (ZDXHAT2D(IIU,IJU))


!		COMPUTE 2D DXHAT   - EDIT THEO BRIVOAL
DO JJ=1,IJU ;
ZDXHAT2D(1:IIU-1,JJ)  = PXHAT2D(2:IIU,JJ) - PXHAT2D(1:IIU-1,JJ)
ZDXHAT2D(IIU,JJ)      = ZDXHAT2D(IIU-1,JJ)
ENDDO;
!
...
ALLOCATE (ZLONUOUT(INO))
ALLOCATE (ZLATUOUT(INO))
ALLOCATE (ZLONVOUT(INO))
ALLOCATE (ZLATVOUT(INO))
ALLOCATE (ZLONTOUT(INO))
ALLOCATE (ZLATTOUT(INO))
ALLOCATE (ZDXHATOUT(INO))
JLOOP1 = 0
...
DO JJ = 1, IJU
  ...
  ZLONUOUT(JLOOP1+1:JLOOP1+IIU) = plonu(1:IIU,JJ)
  ZLATUOUT(JLOOP1+1:JLOOP1+IIU) = platu(1:IIU,JJ)

  ZLONVOUT(JLOOP1+1:JLOOP1+IIU) = plonv(1:IIU,JJ)
  ZLATVOUT(JLOOP1+1:JLOOP1+IIU) = platv(1:IIU,JJ)

  ZLONTOUT(JLOOP1+1:JLOOP1+IIU) = plont(1:IIU,JJ)
  ZLATTOUT(JLOOP1+1:JLOOP1+IIU) = platt(1:IIU,JJ)

  ZDXHATOUT(JLOOP1+1:JLOOP1+IIU) = ZDXHAT2D(1:IIU,JJ) 
  JLOOP1 = JLOOP1 + IIU
  ...
  DO JJ=1,IJU
    DO JI=1,IIU
      ...
      ELSE  ! CEP, ARPEGE (after projection)
         ...
         ZALPHA=ATAN(((ZLONTOUT(JLOOP4)-ZLONVOUT(JLOOP4))*COS(ZLATVOUT(JLOOP4)*(XPI/180)))/(ZLATTOUT(JLOOP4)-ZLATVOUT(JLOOP4)))

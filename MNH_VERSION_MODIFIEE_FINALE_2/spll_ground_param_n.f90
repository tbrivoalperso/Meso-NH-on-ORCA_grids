...
USE MODD_GRID_n,     ONLY : XLON,XLAT,xlont,xlonv,xlatt,xlatv,XXHAT2D, XZZ, XDIRCOSXW, XDIRCOSYW, XDIRCOSZW, &
...
REAL, DIMENSION(:,:), INTENT(INOUT) :: PSEA_UCU,PSEA_VCU !Sea sfc currents!
!
...
REAL, DIMENSION(SIZE(XLON,1),SIZE(XLON,2))  :: YALPHA  ! angle between Y axis and north
REAL, DIMENSION(SIZE(XLON,1),SIZE(XLON,2))  :: ZDXHAT2D  ! DXHAT in 2D
!
...
INTEGER :: IIU,IJU ! Grid size in u,v dimension
!
...
REAL, DIMENSION(:),   ALLOCATABLE :: ZP_UCU      ! u component, surface current
REAL, DIMENSION(:),   ALLOCATABLE :: ZP_VCU      ! v component, surface current
...
IJU=SIZE(XXHAT2D,2)
DO JJ=1,IJU ;
ZDXHAT2D(1:IIU-1,JJ)  = XXHAT2D(2:IIU,JJ) - XXHAT2D(1:IIU-1,JJ)
ZDXHAT2D(IIU,JJ)      = ZDXHAT2D(IIU-1,JJ)
ENDDO;

!	YALPHA(:,:)=ASIN(((4./360.)*XPI*XRADIUS*(XPI/180.)*(xlatt(:,:)-xlatu(:,:)))/ZDXHAT2D(:,:)) 
        YALPHA(:,:)=ATAN(((xlont(:,:)-xlonv(:,:))*COS(xlatt(:,:)*(XPI/180)))/(xlatt(:,:)-xlatv(:,:)))  !EDIT-TBRIVOAL
IF (.NOT. LCARTESIAN) THEN
 ! ZDIR =   ( (XRPK*(XLON(:,:)-XLON0)) - XBETA ) * XPI/180.  + ZALFA
  ZDIR =   YALPHA(:,:) + ZALFA   ! Edit T- BRIVOAL
ELSE
...
PSFV(:,:) = 0.
!
ZWIND(:,:) = SQRT( (ZUA-PSEA_UCU)**2 + (ZVA-PSEA_VCU)**2 )
!
WHERE (ZSFU(:,:)/=XUNDEF .AND. ZWIND(:,:)>0.)
 PSFU(:,:) = - SQRT(ZSFU**2+ZSFV**2) * (ZUA(:,:)-PSEA_UCU(:,:)) / ZWIND(:,:) / XRHODREF(:,:,IKB)
 PSFV(:,:) = - SQRT(ZSFU**2+ZSFV**2) * (ZVA(:,:)-PSEA_VCU(:,:)) / ZWIND(:,:) / XRHODREF(:,:,IKB)
END WHERE
!
...
ALLOCATE(ZP_UCU     (KDIM1D)) 
ALLOCATE(ZP_VCU     (KDIM1D)) 
...
PSEA_UCU (IIB:IIE,IJB:IJE)      = RESHAPE(ZP_UCU(:),      ISHAPE_2)
PSEA_VCU (IIB:IIE,IJB:IJE)      = RESHAPE(ZP_VCU(:),      ISHAPE_2)
!
IF (LDIAG_IN_RUN) THEN

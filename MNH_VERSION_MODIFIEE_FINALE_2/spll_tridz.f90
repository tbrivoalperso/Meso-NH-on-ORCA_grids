...
USE MODD_CST
USE MODD_CONF
USE MODD_GRID_n , ONLY : XDXHAT2D_ll, XDYHAT2D_ll
USE MODD_PARAMETERS
!
USE MODE_ll
USE MODE_IO_ll
...
!
REAL, DIMENSION( SIZE(PMAP,1))  :: ZWORKX  ! work array to compute PDXHATM
REAL, DIMENSION( SIZE(PMAP,2))  :: ZWORKY  ! work array to compute PDYHATM
!
...
DO JI= IIB_ll, IIE_ll
   DO JJ = IJB_ll,IJE_ll!
	PDXHATM=PDXHATM+XDXHAT2D_ll(JI,JJ)
  ENDDO
ENDDO
PDXHATM = PDXHATM /  (REAL(IIMAX_ll)*REAL(IJMAX_ll))
!
!------------------------------------------------------------------------------
!
...
DO JI= IIB_ll, IIE_ll
   DO JJ = IJB_ll,IJE_ll!
	PDYHATM=PDYHATM+XDYHAT2D_ll(JI,JJ)
   ENDDO
ENDDO
PDYHATM = PDYHATM /  (REAL(IIMAX_ll)*REAL(IJMAX_ll))

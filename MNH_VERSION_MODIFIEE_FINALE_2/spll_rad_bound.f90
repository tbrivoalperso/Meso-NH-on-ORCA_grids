...
!
    ...
    END IF
    DO JJ=1,IJE 
       ZCPHASX(JJ,:) = MAX ( 0., MIN ( 1.,                                      & 
                      (-PUT(IIB,JJ,:) + ZPHASX(JJ,:) ) * PTSTEP / PDXHAT2D(IIB,JJ)  )    ) !T-BRIVOAL
...
!
    DO JJ=1,IJE
       ZCPHASX(JJ,:) = MAX ( 0., MIN ( 1.,                                  &
                    ( PUT(IIE+1,JJ,:) + ZPHASX(JJ,:) ) * PTSTEP/PDXHAT2D(IIE,JJ)  )      ) !T-BRIVOAL
    ENDDO
...
!
    DO II=1,IIE
       ZCPHASY(II,:) = MAX ( 0., MIN ( 1.,                                      &
                    (-PVT(II,IJB,:) + ZPHASY(II,:) ) * PTSTEP/ PDYHAT2D(II,IJB)   )      ) !T-BRIVOAL
    ENDDO
!
...
!
    DO II=1,IIE
       ZCPHASY(II,:) = MAX ( 0., MIN ( 1.,                                      &
                    ( PVT(II,IJE+1,:) + ZPHASY(II,:) ) * PTSTEP/PDYHAT2D(II,IJE)  )      ) !T -BRIVOAL
    ENDDO
!

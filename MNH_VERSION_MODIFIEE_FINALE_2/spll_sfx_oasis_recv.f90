...
REAL, DIMENSION(KI),    INTENT(INOUT) :: PSEA_UCU ! Sea u-current stress (Pa)
REAL, DIMENSION(KI),    INTENT(INOUT) :: PSEA_VCU ! Sea v-current stress (Pa)
!TBRIVOAL 2 SST
!
REAL, DIMENSION(KI),    INTENT(OUT) :: PSEA_SST2 ! Sea surface temperature (K)
REAL, DIMENSION(KI),    INTENT(INOUT) :: PSEA_UCU2 ! Sea u-current stress (Pa)
REAL, DIMENSION(KI),    INTENT(INOUT) :: PSEA_VCU2 ! Sea v-current stress (Pa)
!
...
!TBRIVOAL
  IF(NSEA_SST_ID2/=NUNDEF)THEN
    ...
    CALL OASIS_GET(NSEA_SST_ID2,IDATE,ZREAD(:,:),IERR)
    CALL CHECK_RECV(ILUOUT,IERR,YCOMMENT)
    PSEA_SST2(:)=ZREAD(:,1)
  ENDIF
!
  IF(NSEA_UCU_ID2/=NUNDEF)THEN
    ...
    CALL OASIS_GET(NSEA_UCU_ID2,IDATE,ZREAD(:,:),IERR)
    CALL CHECK_RECV(ILUOUT,IERR,YCOMMENT)
    PSEA_UCU2(:)=ZREAD(:,1)
  ENDIF
!
  IF(NSEA_VCU_ID2/=NUNDEF)THEN
    YCOMMENT='Sea v-current stress'
    CALL OASIS_GET(NSEA_VCU_ID2,IDATE,ZREAD(:,:),IERR)
    CALL CHECK_RECV(ILUOUT,IERR,YCOMMENT)
    PSEA_VCU2(:)=ZREAD(:,1)
  ENDIF
!

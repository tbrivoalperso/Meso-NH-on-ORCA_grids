...
!!	T.brivoal   08/02/17 2D Xhat arrays intead of 1D
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
...
!
!-------------------------------------------------------------------------------
! EDIT T BRIVOAL -----> XHAT en 2D
DO JSWB = 1, ISWB
  CALL SURF_SOLAR_SUM     (PXHAT2D, PYHAT2D, ZMAP, PDIRFLASWD(:,:,JSWB), ZENERGY1(JSWB) )
END DO
!
!
...
!              ---------------------------------
!
CALL SURF_SOLAR_SLOPES  (ZMAP, PXHAT2D, PYHAT2D,                 &
...
!              -------------------------------------------
!
CALL SURF_SOLAR_SHADOWS (ZMAP, PXHAT2D, PYHAT2D,           &
                         PCOSZEN, PSINZEN, PAZIMSOL,   &
...
!
DO JSWB = 1, ISWB
  CALL SURF_SOLAR_SUM(PXHAT2D, PYHAT2D, ZMAP,  &
                      ZDIRSWD(:,:,JSWB),   &
                      ZENERGY2(JSWB)       )
  !
  CALL SURF_SOLAR_SUM(PXHAT2D, PYHAT2D, ZMAP,                             &

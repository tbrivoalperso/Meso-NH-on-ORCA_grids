...
REAL, DIMENSION(:,:), ALLOCATABLE     :: ZSEA_UCU ! ocean surface current along x
REAL, DIMENSION(:,:), ALLOCATABLE     :: ZSEA_VCU ! ocean surface current along y
!
...
ALLOCATE(ZSEA_UCU(IIU,IJU))  ! ocean currents
ALLOCATE(ZSEA_VCU(IIU,IJU))  ! ocean currents
ZSEA_UCU=0.
ZSEA_VCU=0.
!
...
!
  ZTIME1 = ZTIME2
!
  CALL SURF_RAD_MODIF (XMAP, XXHAT2D, XYHAT2D,                 &
...
!
  IF( GDCONV ) THEN
    IF (CDCONV == 'KAFR' .OR. CSCONV == 'KAFR' ) THEN
	 ...
	 ZDXDY(:,:)=XDXHAT2D(:,:)*XDYHAT2D(:,:) ! T- BRIVOAL
        !

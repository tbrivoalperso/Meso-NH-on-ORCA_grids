...
ALLOCATE(XXHAT2D(IIU,IJU))
ALLOCATE(XDXHAT2D(IIU,IJU))
ALLOCATE(XYHAT2D(IIU,IJU))
ALLOCATE(XDYHAT2D(IIU,IJU))
...
!              ----------------------------------------
!
CALL SET_GRID(KMI,HINIFILE,HLUOUT,                                       &  
	      IIU,IJU,IKU,NIMAX_ll,NJMAX_ll,                             &
              ...
              XXHAT,XYHAT,XXHAT2D,XYHAT2D,                               &
              XDXHAT,XDYHAT,XDXHAT2D,XDYHAT2D, XMAP,                     &
...
IF (LLG .AND. LINIT_LG .AND. CPROGRAM=='MESONH') &
  CALL INI_LG(XXHAT2D,XYHAT2D,XZZ,XSVT,XLBXSVM,XLBYSVM)
...
ALLOCATE(xlonu(IIU,IJU))
ALLOCATE(xlatu(IIU,IJU))
...
CALL FMREAD(CINIFILE,'latu',CLUOUT,'XY',xlatu,IGRID,ILENCH,YCOMMENT,IRESP)
CALL FMREAD(CINIFILE,'lonu',CLUOUT,'XY',xlonu,IGRID,ILENCH,YCOMMENT,IRESP)
CALL FMREAD(CINIFILE,'latv',CLUOUT,'XY',xlatv,IGRID,ILENCH,YCOMMENT,IRESP)
CALL FMREAD(CINIFILE,'lonv',CLUOUT,'XY',xlonv,IGRID,ILENCH,YCOMMENT,IRESP)
CALL FMREAD(CINIFILE,'latt',CLUOUT,'XY',xlatt,IGRID,ILENCH,YCOMMENT,IRESP)
CALL FMREAD(CINIFILE,'lont',CLUOUT,'XY',xlont,IGRID,ILENCH,YCOMMENT,IRESP)
PRINT*,'FMREAD - OK'

CALL INI_DYNAMICS(HLUOUT,XLON,XLAT,XRHODJ,XTHVREF,XMAP,XZZ,XDXHAT2D,XDYHAT2D,     &
...
IF (CRAD   /= 'NONE') THEN
  ...
  ALLOCATE(XXHAT2D_ll                 (IIU_ll,IJU_ll))
  ALLOCATE(XYHAT2D_ll                 (IIU_ll,IJU_ll))
  ...
  CALL GATHERALL_FIELD_ll('XY',XXHAT2D,XXHAT2D_ll,IRESP)
  CALL GATHERALL_FIELD_ll('XY',XYHAT2D,XYHAT2D_ll,IRESP)

...
USE MODD_SHADOWS_n,  ONLY : XZS_ll, XZS_XY_ll, XXHAT2D_ll, &
                            XYHAT2D_ll, XZS_MAX_ll, XZS_MAX_ll
!
IMPLICIT NONE
!
...
!
      SELECT CASE (JT)
       CASE (1)
         ZX=(5.*PXHAT2D(JI,JJ)+PXHAT2D(JI+1,JJ))/6.
         ZY=0.5*(PYHAT2D(JI,JJ)+PYHAT2D(JI,JJ+1))
         ZZ=(PZS(JI,JJ)+PZS_XY(JI,JJ)+PZS_XY(JI,JJ+1))/3.
       CASE (2)
         ZX=0.5*(PXHAT2D(JI,JJ)+PXHAT2D(JI+1,JJ))
         ZY=(5.*PYHAT2D(JI,JJ+1)+PYHAT2D(JI,JJ))/6.
         ZZ=(PZS(JI,JJ)+PZS_XY(JI,JJ+1)+PZS_XY(JI+1,JJ+1))/3.
       CASE (3)
         ZX=(5.*PXHAT2D(JI+1,JJ)+PXHAT2D(JI,JJ))/6.
         ZY=0.5*(PYHAT2D(JI,JJ)+PYHAT2D(JI,JJ+1))
         ZZ=(PZS(JI,JJ)+PZS_XY(JI+1,JJ)+PZS_XY(JI+1,JJ+1))/3.
       CASE (4)
         ZX=0.5*(PXHAT2D(JI,JJ)+PXHAT2D(JI+1,JJ))
         ZY=(5.*PYHAT2D(JI,JJ)+PYHAT2D(JI,JJ+1))/6.
...
!
!* go to the next grid point encountered by the sun beam ! EDIT T- BRIVOAL -XHAT en 2D
!
        IF (ZSINAZIM>0. .AND. ZCOSAZIM>0.) THEN
          ZDY=(XYHAT2D_ll(II_ll,IJ_ll+1)-ZY)/ZSINAZIM
          ZDX=(XXHAT2D_ll(II_ll+1,IJ_ll)-ZX)/ZCOSAZIM
        ELSE IF (ZSINAZIM<0. .AND. ZCOSAZIM>0.) THEN
          ZDY=(XYHAT2D_ll(II_ll,IJ_ll)  -ZY)/ZSINAZIM
          ZDX=(XXHAT2D_ll(II_ll+1,IJ_ll)-ZX)/ZCOSAZIM
        ELSE IF (ZSINAZIM>0. .AND. ZCOSAZIM<0.) THEN
          ZDY=(XYHAT2D_ll(II_ll,IJ_ll+1)-ZY)/ZSINAZIM
          ZDX=(XXHAT2D_ll(II_ll,IJ_ll)  -ZX)/ZCOSAZIM
        ELSE IF (ZSINAZIM<0. .AND. ZCOSAZIM<0.) THEN
          ZDY=(XYHAT2D_ll(II_ll,IJ_ll)  -ZY)/ZSINAZIM
          ZDX=(XXHAT2D_ll(II_ll,IJ_ll)  -ZX)/ZCOSAZIM
        ELSE IF (ZSINAZIM==0. .AND. ZCOSAZIM<0.) THEN
          ZDX=-(XXHAT2D_ll(II_ll,IJ_ll)  -ZX)
          ZDY=2.*ZDX
        ELSE IF (ZSINAZIM==0. .AND. ZCOSAZIM>0.) THEN
          ZDX=(XXHAT2D_ll(II_ll+1,IJ_ll)-ZX)
          ZDY=2.*ZDX
        ELSE IF (ZSINAZIM<0. .AND. ZCOSAZIM==0.) THEN
          ZDY=-(XYHAT2D_ll(II_ll,IJ_ll)  -ZY)
          ZDX=2.*ZDY
        ELSE IF (ZSINAZIM>0. .AND. ZCOSAZIM==0.) THEN
          ZDY=(XYHAT2D_ll(II_ll,IJ_ll+1)-ZY)
          ZDX=2.*ZDY
        END IF
!
        IF (ZDY<ZDX) THEN
...
!* left triangle
!
          ZXT(1) =0.5*(XXHAT2D_ll(II_ll,IJ_ll)+XXHAT2D_ll(II_ll+1,IJ_ll))
          ZYT(1) =0.5*(XYHAT2D_ll(II_ll,IJ_ll)+XYHAT2D_ll(II_ll,IJ_ll+1))
          ZZT(1) =XZS_ll   (II_ll          ,IJ_ll          ) - ZZCURV
          ZXT(2) =XXHAT2D_ll(II_ll,IJ_ll)
          ZYT(2) =XYHAT2D_ll(II_ll,IJ_ll)
          ZZT(2) =XZS_XY_ll(II_ll  ,IJ_ll  ) - ZZCURV
          ZXT(3) =XXHAT2D_ll(II_ll,IJ_ll)
          ZYT(3) =XYHAT2D_ll(II_ll,IJ_ll+1)
...
!
          ZAT(2) =ZAT(3)
          ZBT(2) =ZBT(3)
          ZXT(3) =XXHAT2D_ll(II_ll+1,IJ_ll)
          ZYT(3) =XYHAT2D_ll(II_ll,IJ_ll+1)

...
!
  ...
  ENDIF
!
  IF(LEN_TRIM(CSEA_SST2)/=0)THEN
    CALL OASIS_DEF_VAR(NSEA_SST_ID2,CSEA_SST2,IPART_ID,IVAR_NODIMS,OASIS_IN,IVAR_SHAPE,OASIS_DOUBLE,IERR)
    IF(IERR/=OASIS_OK) CALL ABOR1_SFX('SFX_OASIS_DEFINE: OASIS def var problem for Sea surface temperature 2')
  ELSE
    NSEA_SST_ID2=NUNDEF
  ENDIF

!
  IF(LEN_TRIM(CSEA_UCU2)/=0)THEN
    CALL OASIS_DEF_VAR(NSEA_UCU_ID2,CSEA_UCU2,IPART_ID,IVAR_NODIMS,OASIS_IN,IVAR_SHAPE,OASIS_DOUBLE,IERR)
    IF(IERR/=OASIS_OK) CALL ABOR1_SFX('SFX_OASIS_DEFINE: OASIS def var problem for Sea u-current stress')
  ELSE
    NSEA_UCU_ID2=NUNDEF
  ENDIF
!
  IF(LEN_TRIM(CSEA_VCU2)/=0)THEN
    CALL OASIS_DEF_VAR(NSEA_VCU_ID2,CSEA_VCU2,IPART_ID,IVAR_NODIMS,OASIS_IN,IVAR_SHAPE,OASIS_DOUBLE,IERR)
    IF(IERR/=OASIS_OK) CALL ABOR1_SFX('SFX_OASIS_DEFINE: OASIS def var problem for Sea v-current stress')
  ELSE
    NSEA_VCU_ID2=NUNDEF
  ENDIF

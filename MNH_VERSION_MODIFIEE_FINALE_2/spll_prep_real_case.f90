...
ALLOCATE(XLAT(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(XLON(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(XXHAT2D(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(XYHAT2D(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))

ALLOCATE(xlonu(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(xlatu(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(xlonv(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(xlatv(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(xlont(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))
ALLOCATE(xlatt(NIMAX+2*JPHEXT,NJMAX+2*JPHEXT))

CALL FMREAD(CPGDFILE,'LAT',CLUOUT,'XY',XLAT,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'LON',CLUOUT,'XY',XLON,IGRID,ILENGTH,HCOMMENT,IRESP)

CALL FMREAD(CPGDFILE,'latu',CLUOUT,'XY',xlatu,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'lonu',CLUOUT,'XY',xlonu,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'latv',CLUOUT,'XY',xlatv,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'lonv',CLUOUT,'XY',xlonv,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'latt',CLUOUT,'XY',xlatt,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'lont',CLUOUT,'XY',xlont,IGRID,ILENGTH,HCOMMENT,IRESP)



!Adding 2D XHAT variables

CALL FMREAD(CPGDFILE,'XHAT2D',CLUOUT,'XY',XXHAT2D,IGRID,ILENGTH,HCOMMENT,IRESP)
CALL FMREAD(CPGDFILE,'YHAT2D',CLUOUT,'XY',XYHAT2D,IGRID,ILENGTH,HCOMMENT,IRESP)
...
ELSE IF (YATMFILETYPE=='GRIBEX') THEN
  IF(LEN_TRIM(YCHEMFILE)>0 .AND. YCHEMFILETYPE=='GRIBEX')THEN
    CALL READ_ALL_DATA_GRIB_CASE('ATM1',YPRE_REAL1,YATMFILE,YPGDFILE,ZHORI,NVERB,LDUMMY_REAL,XLAT,XLON,XXHAT2D,XYHAT2D,xlatu,xlonu,xlatv,xlonv,xlatt,xlont)
  ELSE
    CALL READ_ALL_DATA_GRIB_CASE('ATM0',YPRE_REAL1,YATMFILE,YPGDFILE,ZHORI,NVERB,LDUMMY_REAL,XLAT,XLON,XXHAT2D,XYHAT2D,xlatu,xlonu,xlatv,xlonv,xlatt,xlont)
  END IF
...
IF(LEN_TRIM(YCHEMFILE)>0)THEN
  ...
  CALL READ_ALL_DATA_GRIB_CASE('CHEM',YPRE_REAL1,YCHEMFILE,YPGDFILE,ZHORI,NVERB,LDUMMY_REAL,XLAT,XLON,XXHAT2D,XYHAT2D,xlatu,xlonu,xlatv,xlonv,xlatt,xlont)
...
ALLOCATE(XDXHAT2D(SIZE(XXHAT),SIZE(XYHAT)))
ALLOCATE(XDYHAT2D(SIZE(XXHAT),SIZE(XYHAT)))
...
!                   XMAP,XLAT,XLON,XDXHAT,XDYHAT,XZZ,ZJ       )
  ...
  CALL SM_GRIDPROJ(CLUOUT0,XXHAT,XYHAT,XZHAT,XXHAT2D,XYHAT2D,XZS,            &
                   LSLEVE,XLEN1,XLEN2,XZSMT,XLATORI,XLONORI,XLAT,XLON,xlonu,xlatv,xlatt,    &
                   XMAP,XDXHAT,XDYHAT,XDXHAT2D,XDYHAT2D,XZZ,ZJ)
...
!XDXHAT2D(:,:)=24416
!XDYHAT2D(:,:)=20535
!XXHAT2D(:,:)=XXHAT2D(:,:)*XMAP(:,:)
!XXHAT2D(:,:)=XXHAT2D(:,:)*XMAP(:,:)
!
CALL METRICS(XMAP,XDXHAT,XDYHAT,XDXHAT2D,XDYHAT2D,XZZ,XDXX,XDYY,XDZX,XDZY,XDZZ)
!
...
IJU_ll=NJMAX_ll + 2 * JPHEXT
  ALLOCATE(XDXHAT2D_ll                 (IIU_ll,IJU_ll))
  ALLOCATE(XDYHAT2D_ll                 (IIU_ll,IJU_ll))
  ALLOCATE(XMAP_ll                 (IIU_ll,IJU_ll))
  ALLOCATE(XLAT_ll                 (IIU_ll,IJU_ll))
  ALLOCATE(XLON_ll                 (IIU_ll,IJU_ll))

  CALL GATHERALL_FIELD_ll('XY',XDXHAT2D,XDXHAT2D_ll,IRESP)
  CALL GATHERALL_FIELD_ll('XY',XDYHAT2D,XDYHAT2D_ll,IRESP)
  CALL GATHERALL_FIELD_ll('XY',XLAT,XLAT_ll,IRESP)
  CALL GATHERALL_FIELD_ll('XY',XLON,XLON_ll,IRESP)
  CALL GATHERALL_FIELD_ll('XY',XMAP,XMAP_ll,IRESP)
...
IF (YATMFILETYPE=='GRIBEX') THEN
  CSURF = "EXTE"
  DO JRR=1,NRR
    IF (JRR==1) LUSERV=.TRUE.
    IF (JRR==2) LUSERC=.TRUE.
    IF (JRR==3) LUSERR=.TRUE.
    IF (JRR==4) LUSERI=.TRUE.
    IF (JRR==5) LUSERS=.TRUE.
    IF (JRR==6) LUSERG=.TRUE.
    IF (JRR==7) LUSERH=.TRUE.
  END DO
END IF
!

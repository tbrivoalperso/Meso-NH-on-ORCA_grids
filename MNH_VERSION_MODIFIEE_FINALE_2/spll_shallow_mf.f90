...
USE MODD_GRID_n, ONLY : XDXHAT2D, XDYHAT2D
...
ENDIF
     ...
     IF (HMF_UPDRAFT == 'BOUT') THEN
      ...
      !! calcul de Dx/(h+hc)
      DO JI=1,SIZE(XDXHAT2D(:,1))
       DO JJ=1,SIZE(XDYHAT2D(1,:))
          ZRESOL_GRID((JJ-1)*SIZE(XDXHAT2D(:,1))+JI)=SQRT(XDXHAT2D(JI,JJ)*XDYHAT2D(JI,JJ))
       ENDDO
      ENDDO
